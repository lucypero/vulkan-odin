...
if not os.path.isfile(c_output_path+"vulkan_core.h"):
    ...
    with open(c_output_path+"vulkan_core.h", "w") as f:
        f.write(src)
if not os.path.isfile(c_output_path+"vk_platform.h"):
    ...
    with open(c_output_path+"vk_platform.h", "w") as f:
        f.write(src)
if not os.path.isfile(c_output_path+"vk_layer.h"):
    ...
    with open(c_output_path+"vk_layer.h", "w") as f:
        f.write(src)
if not os.path.isfile(c_output_path+"vk_icd.h"):
    ...
    with open(c_output_path+"vk_icd.h", "w") as f:
        f.write(src)
if not os.path.isfile(c_output_path+"vulkan_win32.h"):
    ...
    with open(c_output_path+"vulkan_win32.h", "w") as f:
        f.write(src)
if not os.path.isfile(c_output_path+"vulkan_metal.h"):
    ...
    with open(c_output_path+"vulkan_metal.h", "w") as f:
        f.write(src)
if not os.path.isfile(c_output_path+"vulkan_macos.h"):
    ...
    with open(c_output_path+"vulkan_macos.h", "w") as f:
        f.write(src)
if not os.path.isfile(c_output_path+"vulkan_ios.h"):
    ...
    with open(c_output_path+"vulkan_ios.h", "w") as f:
        f.write(src)

src, win32_src = "", ""
with open(c_output_path+"vulkan_core.h", "r") as f:
    src = f.read()
with open(c_output_path+"vulkan_win32.h", "r") as f:
    win32_src = f.read()
    src += win32_src
...
def group_functions(f):
    ...
    f.write("// load all the functions that do not require an instance to be loaded\n")
    f.write("// call this AFTER vk.GetInstanceProcAddr is loaded\n")
    f.write("load_proc_addresses_no_instance :: proc() {\n")
    for group_name, group_lines in group_map.items():
        # only write Loader Procedures here
    ...
    f.write("// load all the functions that require a created instance to be loaded\n")
    f.write("// call this AFTER vk.GetInstanceProcAddr is loaded and you created a vulkan instance\n")
    f.write("load_proc_addresses_with_instance :: proc(instance: ^Instance) {\n")
    for group_name, group_lines in group_map.items():
        # write all procs except Loader procs here
        if group_name != "Loader":
            max_len = max(len(name) for name, _ in group_lines)
            for name, vk_name in group_lines:
                k = max_len - len(name)
